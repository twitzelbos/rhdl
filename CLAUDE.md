...
- **`combinational-reachability-and-loop-detection.md`** — plan to compute a per-widget combinational input-output reachability matrix during descriptor finalization, and use it for composition-level combinational-loop detection that runs *before* RHIF lowers to RTL/NTL. Subsumes the existing `circuit::drc::no_combinatorial_paths` DRC (refactored to query the matrix), complements the existing NTL-level `ReorderInstructions` Kahn's-algorithm loop detector (kept as a safety-net backstop), and produces materially better diagnostics by reporting cycles at the widget-composition level rather than at flattened-opcode level. **Decision recorded:** the matrix has four sub-matrices (i_to_o, i_to_d, q_to_o, q_to_d) covering every combinational path that matters for cycle detection across the widget hierarchy. It is computed bottom-up over the widget tree, recursive, deterministic, and cached by hash key. The matrix is also the metadata that `package-manager-architecture.md` §9 needs for cross-crate clock-domain consistency claims, AND the input that all three phases of `auto-pipelining-plan.md` consume (Phase 1 reads `i_to_o` for cut-point identification; Phase 2 reads all four matrices for hazard analysis and Carloni-LID-compliant relay-station insertion; Phase 3 builds on the composition-level cycle detector for II analysis). **Sequencing constraint:** this work should therefore land before Package Manager Phase 2 *and* before any auto-pipelining phase begins; otherwise the auto-pipeliner re-derives matrix-equivalent information and the two analyses risk drift. Three phases: matrix computation and integration into Descriptor; refactor `no_combinatorial_paths` to query the matrix; add composition-level cycle detection with the new diagnostic and update `logic_loop.rs` to expect it. Total ~5-7 weeks of compiler work. Field-path-level analysis in v1; bit-level is a v2 stretch.
- **`black-box-connectivity.md`** — design for how a module RHDL did **not** author — a vendor primitive, an IP core, a hand-written Verilog block — declares its combinational connectivity, so the reachability analyses are sound in its presence. **Decision recorded:** connectivity is *declared*, never inferred from behavioural Verilog, and the default for an undeclared module inverts from "no feedthrough" (today's optimistic and unsound assumption, inherited from a `continue` statement in `ntl::graph::make_net_graph` that skips every `BlackBox` op) to "every input reaches every output". Declaration is primarily a Rust-level `BlackBoxConnectivity` on `with_netlist_black_box`; a RON file format plus a build script exists for bulk vendor libraries, read at *compile* time because RHDL compiles at run time and a runtime path would resolve against whatever binary called `descriptor()`. Interfaces can be cross-checked against real Verilog because `rhdl-vlog` parses it (`syn::parse_str::<ModuleDef>`); paths cannot, and are a judgement. **Also documents how external modules get in today**, which is worth reading before assuming: `Driver<T>` at the fixture boundary (outside every analysis — the okHost driver mounts 11 inputs and 7 outputs and nothing checks it), `with_netlist_black_box` for Verilog RHDL writes itself, and Vivado IP that exists only as TCL with no circuit-level representation at all. Should land before the first vendor primitive, not after.
- **`farrow-resampler.md`** — scoping plan for a fractional-rate resampler, which is what removes the CIC chain's integer-rate constraint. **Decision recorded:** the resampler goes **after** the CIC doing only a fine ratio trim, not before it and not instead of it. Measured: a cubic Lagrange Farrow at the envelope rate, where the signal occupies 0.2 of the sample rate, errs by 0.46 dB and 3.8° — which falsifies phase; the same filter after a CIC whose integer runtime rate lands the intermediate within `1 + 1/R` of the target sees the signal at 0.0016 of the sample rate and errs by nothing measurable. The coarse integer step and the fine fractional trim each do the thing the other is bad at, and a Lagrange interpolator's poor stopband is why it must not do the bulk rate change. **The phase contract is the substance of the document** — six clauses, each with its hazard, closure and test: an exact fixed-point accumulator exposed on the output (the group delay varies per sample by design, so a consumer that cannot read it cannot correct for it); one accumulator serving both quadratures structurally rather than two widgets that might differ; DC gain exactly one at every `μ`, **which coefficient quantisation destroys unless each branch's integer sum is trimmed to exactly `1 << SHIFT` or exactly zero** — the failure mode is a discrete spur at the resampling beat frequency, not broadband noise, so a noise measurement will not find it; a reported error bound; a framing mark defined as emerging on the first output at or after the marked input's time; and a group delay stated in both its constant `(K-1)/2` part and its varying `μ` part.
  **Amended (§0) on the question of whether a better filter exists and the observation that this resampling happens upstream of the DUC and could run in software.** It can, and then most of the plan is the wrong one. Measured: windowed sinc is far better *wideband* — 32 taps give 0.0003 dB and 0.0004° at 0.2 of the sample rate where cubic Lagrange gives 0.457 dB and 3.8° — while Lagrange is better *narrowband* per tap, being maximally flat at DC. And spectral resampling (FFT, zero-pad, inverse FFT) is exact at **−265 dB**, a hundred dB better than any FIR, with phase exact by construction; its only weakness is block-edge error, which is bounded to the boundaries and is what overlap-save fixes. **So the cheapest architecture contains no resampler:** the host resamples in software to exactly `fs/R` for an integer `R` and the FPGA interpolates by `R` with the existing CIC. Better still, if the envelope is *synthesised* rather than resampled, evaluate the waveform at the output instants and resample nothing. The hardware widget is reserved for a live source at an unrelated clock, a continuously-varying rate, or a closed loop tracking a recovered clock — for a host-driven transmit envelope the software route wins on every axis.
- **`xilinx-primitive-library.md`** — execution plan for a complete, *verified* black-box declaration library for the Xilinx 7-series (the primitives a Zynq-7020 has), to be carried out on a machine with Vivado installed. Steps needing the vendor tools are marked **[VIVADO]**. **Decision recorded:** the library is *extracted* from `$XILINX_VIVADO/data/verilog/src/unisims/` rather than written from memory, because a connectivity declaration is *believed* by the analysis — a primitive wrongly declared `None` is a combinational path the compiler asserts does not exist, arriving in a checked-in file citing UG953. Connectivity is then **tested** by simulating each primitive with every clock held static and toggling inputs: observing an output change *proves* a path, so the harness can refute an unsound `None`, which is the direction that hides a real loop. It cannot confirm absence, and the plan says so. Attribute-dependent primitives (`DSP48E1`'s `PREG`, block RAM's `DO_REG`) declare the *most combinational* configuration, so a mis-set attribute costs a spurious report rather than a hidden path. **Blocked on one prerequisite:** RHDL cannot currently instantiate a module it did not define, because `ModuleList::checked()` runs iverilog and it rejects `Unknown module type`; the fix is port-only stubs generated from the declarations themselves, prototyped in §2.1. Also plans driver/fixture connectivity — note `IBUFDS` is a buffer, so its honest declaration is a *path*, not null. **§7 covers device-specific and multi-platform code**, which is the other half of shipping a vendor library: **decision recorded** that `Target`'s capability methods (`vendor-primitive-architecture.md` §3.2) are the wrong shape for a primitive asked for by *name* — two hundred of them, no meaningful default for `PS7` — so a named primitive's device families are carried in its black-box declaration as data, and a design's `Requirement` is computed by walking the constructed widget tree and checked at `hdl_for(&target)`. Widgets stay free of a target generic. A widget that wants different primitives per target picks at *construction*, not at emission, because the Rust simulator has no target and a widget holding both alternatives would model one branch while emitting the other; the branch belongs at a black-box leaf, and widgets above it stay target-agnostic. Cargo features are the wrong mechanism (additive and unifying, and RHDL compiles at run time so a `#[cfg]` cannot be consulted at `hdl_for`) except for the one thing they suit: whether the generated library data is compiled in at all.
