...
**Phase 3.5 — Boot-to-OS-loader chain (3-5 weeks).** The "what does it actually take to land in the OS loader" gap that Phase 3's one-line summary hides. Each arrow below is a real implementation gap that surfaces after the disk subsystem is wired and the chip starts running PROM-loaded microcode:

  1. **Microengine + 16-task arbiter executes spec-correct cycles.** F1=TaskYield is K+2 (per AltoHW §2.4 — task register loads at end of cycle K, "one additional instruction is executed before the switch becomes effective", new task runs at K+2). F2 NEXT-modifiers compose with the dispatch/branch register correctly. Single-driver rule-arbiter for `task_mpc[k]` (no two rules write the same slot in the same cycle).
  2. **PROM-resident initial microcode runs.** Reset-vector microinstruction at MPC=0x000 (NOVEM), through the Emulator-task boot stub at 0x152..0x154, into a TaskYield that hands control to whoever the priority arbiter says is highest-woken.
  3. **KSEC (Disk Sector task) takes the first cycle of real I/O.** Sector mark fires (Diablo 31 has a hardware sector pulse on every sector boundary, ~6.94 ms apart at 1500 rpm × 12 sectors/track). Disk Sector wakeup latches. KSEC runs its head-position-check, address-match, and seek-completion sequence, then yields back.
  4. **KWD (Disk Word task) drains the bit-stream off the disk.** Word-shift register fills 16 bits, raises Disk Word wakeup, KWD runs, writes to memory via the M-channel, repeats for the boot-block-sized read (Alto boot block = 1 sector = 256 16-bit words = 512 bytes).
  5. **Memory subsystem returns the right words.** M-channel memory-read pipeline (3-cycle latency on real Alto), `MD←` writes, address-register updates, M-register-as-pointer semantics. Boot block lands at memory location 0x0001..0x0100 (low Alto memory).
  6. **Emulator task wakes and starts executing the boot block as Nova-emulated instructions.** Emulator's NOVEM/NOVEMM dispatch picks up the IR at the boot PC (= virtual address 0x0001 → first word of loaded block), decodes one of the 8 Nova opcode classes (LDA, STA, ISZ, DSZ, JMP, JSR, arithmetic-logic, I/O), executes the microcode handler. R[6]=PC, R[3..0]=AC0..AC3 (XOR-3), R[8]=XH (BLT loop counter), R[5]=SAD, R[4]=NWW.
  7. **The boot block executes the standard Alto bootstrap.** `BLT` / `MOVB` style block-copy of the OS loader from disk to a higher memory region; jumps to the loader entry point.
  8. **OS loader runs.** Driven by the Emulator task with the same Nova-emulated dispatch. Reads more sectors via KSEC/KWD, stages the OS image into memory, sets up display/cursor task initial state.
  9. **First on-screen artifact appears.** Display Word task starts walking the framebuffer, Display Horizontal/Vertical tasks generate the timing signals, the loader's prompt or boot screen lands in the framebuffer. This is the first directly-observable "the Alto is booting" moment in our framebuffer dump.

Estimated dev-days for the chain *given Phase 3 done*: ~15-25 dev-days. Most of that is in steps 4-6 (memory pipeline + Nova dispatch + boot-block execution). Each step is independently testable with the lockstep harness against ContrAlto. Steps 1-2 are essentially "lockstep clean from cycle 0", step 3 is "task arbiter switches to KSEC at the right cycle", step 4 is "boot block bytes match", step 5 is "memory state matches at every M-channel completion", step 6 is "first Nova IR fetch matches", step 7 is "BLT writes match in lockstep", step 8 is "first OS-loader IR fetch matches", step 9 is "framebuffer hash matches at first display-Word completion".

This is the level of granularity that distinguishes "we have a microengine" from "we boot real Alto software". When the chain is documented as one Phase 3 line, the engineer estimating the work underestimates by 5-10× — hence Phase 3.5.
