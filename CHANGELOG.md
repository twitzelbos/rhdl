...
- **I nearly dropped every edge from an op feeding a black box, and then overstated what that would have cost.** The helper resolving a register to its graph node returned `None` for `WriteSource::OpCode`, so only black boxes fed *directly* from the top-level inputs would have been connected. Caught while re-reading the diff rather than by a test.

  The first version of this entry called that a false "no combinational path". It would not have been. `make_net_graph` has exactly two callers, both in `drc.rs`: the span-finding walk that runs only on the error path, and `feedthrough_by_netlist_walk`, the oracle the corpus cross-check compares against. Since Phase 2 the DRC's *verdict* comes from the reachability matrix, which is computed by an entirely different route — so the real cost would have been a diagnostic that lost its spans, plus a cross-check oracle quietly disagreeing with the thing it exists to check. Bad, and not the same as a wrong answer.

  Worth recording because the correction came from writing the test rather than from thinking harder: the first test asserted `no_combinatorial_paths` reports the path, it passed with the fault reintroduced, and that is what showed the fault could not reach the verdict at all.
...
**Validation:** the acceptance criterion had two halves and both are met, and the graph builder's operation-source branch is covered by a test that was *watched to fail* with the fault reintroduced — a test for a fixed bug being worth nothing until then. Nothing moved: 1377 `rhdl-fpga` tests pass with no HDL snapshot, VCD digest or diagnostic re-blessed, so the eight declarations state what the analysis already assumed. And the thing that could not be caught before now is: four new tests use one black-box inverter whose Verilog is `assign o = ~i;`, parameterised on what it *claims*, so the same module is tested as an honest combinational box and as one claiming to be registered. The honest one is reported as a feedthrough by `no_combinatorial_paths`; a ring of two is reported as a two-widget combinational cycle; the claiming-registered versions of both are accepted. Before this change the ring built without complaint.

### Coverage audit before Phase 2, same PR

Asked what else was uncovered before moving on, and the honest answer was three things — one of which turned out to be a designed diagnostic that could never fire.

- **`BlackBoxConnectivity::Opaque` was never constructed anywhere**, in the tree or in a test, so `to_matrix`'s and `connect_all`'s handling of it was dead code. Phase 2 makes it the *default*, so shipping it unexercised and then promoting it would have built on a path nothing had ever run. Now covered both ways: an opaque black box connects every cell of `i_to_o` and is followed by the netlist graph, and a ring of two is reported as a cycle.
- **`BlackBoxPortNotFound` could not fire.** The design document justifies `Path` over a bare port-name string on the grounds that "a typo in a port name becomes a diagnostic rather than a silently missing edge". It did produce a diagnostic — the wrong one. `to_paths` runs before `to_matrix` and its `bit_range` failed first with a bare "RHDL Path Error", so the error written to name the offending port was unreachable. Found by writing the negative test §11.1 asks for, and fixed by mapping the failure in `to_paths` too.
- **The asynchronous route had no cycle test.** Every ring was `Synchronous`. The two differ where it matters: an asynchronous kernel is `fn(i, q)` rather than `fn(clock_reset, i, q)`, so `i` sits at a different port index, and reading the wrong one yields an all-false matrix — indistinguishable from a design with no feedthrough. Verified by breaking the index and watching the new test fail.

Each of the three new tests was checked by reintroducing the fault it guards and confirming it fails. That is also how the previous entry's claim about the graph-builder bug got corrected: the first version of that test passed with the fault present, which is what revealed the fault could not reach the DRC's verdict at all.

**Still uncovered, and pre-existing rather than introduced here:** `no_combinatorial_paths` is `<T: Synchronous>`, so there is **no feedthrough DRC for asynchronous circuits**. Cycles in them are caught, feedthroughs are not asked about.
...
**Why this, why now:** CLAUDE.md §8 lists `cargo clippy --all -- -D warnings` as required before any commit, and it did not pass. A required check that nobody can pass is worse than no check: it trains everyone to skip the step, so the one real lint hiding among the noise never gets seen.

**Design decisions:**

- **Three lints are suppressed in the `#[kernel]` macro, not at the call sites.** `match_like_matches_macro`, `field_reassign_with_default` and `needless_range_loop` all suggest constructs the kernel subset rejects — verified by compiling each: `matches!` gives "Unsupported expression type matches!" because the subset takes no macros; `..Default::default()` fails E0283 in kernel position; `a.iter().enumerate()` fails because there are no references. That is 42 lints across many files for one structural reason, and it applies to every kernel written from now on. The macro already allowed `manual_map`, `redundant_pattern_matching` and `manual_memcpy` for exactly this reason, so this extends an established list rather than inventing a mechanism.
- **Where clippy was wrong about deliberate code, it gets an `#[allow]` and a reason, not a rewrite.** Three `if_same_then_else` sites are decode tables — the Alto microengine's F1 map, where codes 9 and 10 genuinely produce the same result, and the RV32I CSR read where `mhartid` reads zero because the core is single-hart while the fallback reads zero because an unimplemented CSR must. Merging those arms deletes the distinction the table exists to record. Likewise `0xC0 | 0x00` in the 16550 IIR table keeps the bit-field layout legible, and `data_oe = false` stays as a declared default so that a path added later which forgets to assign it is not driving a tristate bus by accident.
- **The remaining 225 are documentation, and are deliberately not in this change.** All of them are `missing_docs` on struct fields and enum variants, concentrated in `serial_bus` protocol PHYs — `can_master.rs` alone has 45. CLAUDE.md §6 Layer B says the `In`/`Out` field comments *are* the protocol's documentation, which makes this a writing task, not a lint pass. Filling 225 slots with "/// The clk field." would satisfy the linter and leave the codebase worse.

**Surprises and gotchas:**

- **My first count was wrong by a factor of eighteen.** I reported "~24 lints, mostly mechanical" and the real figure was **432**. The cause is that `-D warnings` promotes them to errors, and rustc stops after a few errors per crate — so the list I read was truncated at the first handful of each crate and I mistook it for the whole. Counting warnings (`cargo clippy --all --message-format=short`, no `-D`) is what gives the real number. The user had already chosen this work on the strength of the wrong figure.
- **`cargo clippy --fix` produces code that does not compile, and silently reverts.** Run against `rhdl-fpga` it applies every machine-applicable suggestion, discovers the crate no longer builds, rolls the whole thing back and reports `errors present after applying fixes`. It is not usable as a batch tool here. Applying one lint at a time works, and `--fix` needs `-- --no-deps`; passing lint-level flags after `--` displaces that argument and the run silently fixes nothing.
- **Two of the lints were real bugs in documentation.** `rhdl-rule/src/lib.rs` had a list-item continuation line starting with `+`, which markdown renders as a nested bullet rather than the sentence it was meant to be. And 25 of the 31 unused imports were the same `use rhdl::core::fsm::analysis::Transition` line: leftovers from a real migration, where each FSM widget used to define the author-curated `FSM_TRANSITIONS` constant that CLAUDE.md §12 rule 14 describes, until the RHIF extraction pass in `rhdl-core::fsm::extraction` made it unnecessary. The constants went and the imports stayed. Rule 14 is now stale documentation — worth a separate look.
- **A lib-only clippy pass is not a safe basis for deleting imports, and I deleted three that were needed.** `cargo clippy --all` does not build test targets, so `use crate::lid::carloni::{self, Carloni}` in the three `rcstream` relay/AXI-bridge files reported `self` as unused — while the `#[cfg(test)]` module in each uses `carloni::Out`. The lib kept compiling and the *test* build broke, which the first full-suite run reported as `cannot find module or crate carloni` and zero tests executed. Worse, that failure aborted the run early, so any further breakage of the same kind was hidden behind it; the check that actually settles it is `cargo test --all --no-run`, which compiles every test target. The fix is not to restore the file-level import but to scope it to the test module, which is true to where it is used and keeps the lint quiet.
- **The macro change had to be proved inert, and was.** A change to generated code could shift lowering, so the evidence is that all 1377 `rhdl-fpga` tests pass without re-blessing a single `expect_test` snapshot and the tree is clean afterwards. If any Tier-3 HDL snapshot had moved, the change would not be what it claims.

**Validation:** `cargo clippy --all -- -D warnings` reports **0 non-documentation errors**, down from 432 total. `cargo test --all --no-run` compiles every test target (the check that catches import removals a lib-only pass cannot see), and `cargo test --all --no-fail-fast` is green with the tree clean afterwards. The kernel-subset claims are each backed by a compile: three throwaway kernels written to confirm that `matches!`, `..Default::default()` and `.iter().enumerate()` are actually rejected, rather than assumed to be. `+=` and `<<=` *are* accepted, and the assign-op rewrite touching 34 kernel sites was checked by comparing the compound and explicit forms across all 256 inputs of an 8-bit kernel before applying it.

**Follow-ups:**

- 225 `missing_docs` across 32 files, mostly `serial_bus` field and variant documentation. Until they are written the gate still fails, so §8 remains aspirational.
- CLAUDE.md §12 rule 14 describes the author-curated `FSM_TRANSITIONS` constant that the RHIF extraction pass replaced. No widget defines one any more.
...
**Why this, why now:** three loose ends left by the `cic_chain!` work, each one a place where the tooling claimed more than it delivered. The search only ever considered two CIC stages; the report drew an equiripple design identically to a least-squares one, hiding the only thing that distinguishes them; and the book's internal links were checked by whoever remembered to check them.

**Design decisions:**

- **`allow_cascade: bool` became `max_chain_stages: usize`.** A boolean can only say "one stage or two", and two was never a principled ceiling — it was the shape of the enumeration code, which broke `R` at its square root and paired the factors. `ordered_factorisations` replaces that with a recursion over ordered factor lists bounded by the budget, so raising the ceiling is a parameter rather than a rewrite. Default 3.
- **A third stage genuinely pays, and the test says where.** The first version of the test asserted only `three.cost <= two.cost`, which is true by construction — the two-stage options are a subset of what a three-stage search considers, so the assertion could not fail and proved nothing. Replaced with a strict win on `/1024` at 16 kHz, where `[128, 8]` at cost 65.0 gives way to `[4, 32, 8]` at 49.0, plus a monotonicity test that a larger budget never costs more. An `#[ignore]`d survey prints the whole picture: at 16 kHz the third stage wins at `/256`, `/488`, `/512`, `/1024` and `/1920`.
- **When a stopband is requested, the survey plot spans the full output Nyquist.** Both plots used to stop at the passband edge, which is the one band a stopband requirement says nothing about — so a Remez report and a least-squares report were the same picture, and the achieved attenuation appeared nowhere but a line of text. The zoomed plot still covers the passband, so widening the survey costs nothing.
- **The stopband markers name their curve, in its colour.** `stopband_db` measures *the filter alone*; the composite sits well below it because the cascade contributes its own rolloff. A line labelled "got 66.0 dB" against a composite that visibly reaches −90 reads as an error of 20 dB or more, so the achieved line is drawn in the compensator's colour, labelled "compensator 66.0 dB", and the figures block says which filter the number describes.
- **The book's structure is checked by `cargo test`, not by CI.** The original follow-up was a CI job running `mdbook build`. There is no CI, and `mdbook` is not installed in every environment this repository is built in, so the check would have been aspirational. Four tests in the book's own `code` crate verify every `SUMMARY.md` link resolves, every `{{#rustdoc_include path:anchor}}` names a real file and a real anchor, and no chapter is orphaned — no rendering required.
- **Dead chapters are allowlisted, not deleted.** The ten orphans are abandoned drafts from upstream; removing them from this fork would make every future merge noisier for no reader's benefit. Each is named with why it is dead, and a fourth test fails if an allowlisted file is deleted or finally linked, so the list cannot outlive what it excuses.

**Surprises and gotchas:**

- **The book test found four broken includes on its first run.** `advanced.md` asked for `one-hot-svg-test` where the anchor is `one-hot-test`; `edge_time.md` asked for `edge-time-demo`, which was never marked; and `derive.md` quoted `circuit_io_trait` and `timed_trait`, which were never written. A missing anchor renders as an empty code block — it does not fail a build, and it looks like the author meant it. Three chapters had been shipping with a hole in them.
- **A 47-tap design ran off the right edge of the page.** The tap list was formatted with `{:?}` and simply overflowed, with the last coefficients absent and the report looking complete. Equiripple designs with a stopband requirement routinely need forty-plus taps, so this stopped being a corner case the moment the anti-alias report existed. `wrap_values` measures against real Helvetica metrics and breaks the list, indenting continuations under the first value.
- **Writing the report's first tests found three more defects.** `report.rs` had none — the 51 tests in this area were all in `pdf.rs` and `plot.rs`, so both the wrapping and the whole stopband branch were unguarded. The tests caught, in order: the wrap dropping its separator at the line break, so `..., 3975` / `-17439, ...` has no comma between those two values and copying the list out silently merges them; `stopband_db` returning infinity when the edge leaves no band above it — which `chain::design` accepts, since infinity satisfies any requested level — rendering as `stopband .......... inf dB`, which reads as a spectacular result rather than as a requirement that measured nothing; and the axis floor being derived from the asked-for level alone, so a design that overshoots badly (asked 40 dB, got 90) has its achieved line clamped onto the frame floor. Two of the three would have shipped in a committed PDF.
- **A missing axis label was not a bug.** The zoomed plot's last tick reads 0.225 on an axis that looks like it should reach 0.25, which I took for floating-point loss in `nice_ticks`. The axis actually ends at 0.24986 — 64 kHz against a 128.0738 kHz Nyquist — so 0.25 is genuinely outside it. `nice_ticks` was right; reading the render was not the same as reading the range.
- **The one report that changed is the one whose search widened.** `cic_chain_report.pdf` differs by a single line: its runner-up is now `[4, 61, 2]` at cost 96.8 rather than `[122, 4]` at 97.3. That the rest is byte-identical is the evidence that widening the survey plot is inert when no stopband was asked for.
- **`cargo clippy --all -- -D warnings` already fails on `main`** — 7 errors in `rhdl-dsp-design` and 20 in `rhdl-rule-core`, none of them from this change. The lint in the line this change added is fixed, bringing `rhdl-dsp-design` to 6; the rest is a separate cleanup and is called out here rather than folded in silently.

**Validation:** 63 tests in `rhdl-dsp-design` plus the `#[ignore]`d survey; 4 new book-integrity tests; 9 new tests in `report.rs`, which had none. `cargo test --all --no-fail-fast` green, tree clean afterwards, all three committed PDFs byte-identical across runs. The anti-alias report was read page by page as a rendered image rather than extracted as text — which is how the tap overflow and the mislabelled stopband lines were found, and text extraction would have shown neither.

**Follow-ups:**

- `cargo clippy --all -- -D warnings` fails on `main` (27 errors across `rhdl-dsp-design` and `rhdl-rule-core`). CLAUDE.md §8 lists it as required before any commit, so either the lints get fixed or the requirement is not real.
- The stopband metric describes the compensator alone. A composite figure — what the cascade and filter achieve together above the stopband edge — would be the more useful number to specify against, and would let the search trade CIC stages against taps.
