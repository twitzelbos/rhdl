...
fn software_writes_msip_to_trigger_self_interrupt() {
    ...
    assert_eq!(
        mem[0], 0x8000_0003,
        "software MSIP should fire as M-software interrupt"
    );
}
...
fn software_writes_msip_pipelined_parity() {
    ...
    assert_eq!(
        ...
        "pipelined MSIP-via-software parity"
    );
}
...
fn software_can_clear_msip() {
    ...
    assert_eq!(
        mem[0], 0x55,
        "after clearing MSIP, no interrupt should fire"
    );
}
...
fn vectored_mtvec_routes_msi_to_offset_c() {
    ...
    assert_eq!(
        mem[0], 0xBB,
        "vectored MSI should land at base + 0xC, not at base"
    );
}
...
fn vectored_mtvec_sync_exception_still_to_base() {
    ...
    assert_eq!(
        mem[0], 0xCC,
        "ECALL (sync) goes to base even in vectored mode"
    );
}
...
fn sim_vectored_mtvec_routes_to_offset() {
    ...
    assert_eq!(
        cpu.pc,
        0x40 + 4 * 3,
        "PC should be at vectored offset for cause 3"
    );
}
