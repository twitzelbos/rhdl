...
/// Bundled internal state for the 1-Wire master (CLAUDE.md §3.1).
#[derive(PartialEq, Debug, Digital, Clone, Copy, Default)]
pub struct OneWireMasterExtras<const T_W: usize>
where
    rhdl::bits::W<T_W>: BitWidth,
{
    pub tick: Bits<T_W>,
    ...
    rhdl::bits::W<T_W>: BitWidth,
{
    ...
    extras: dff::DFF<OneWireMasterExtras<T_W>>,
    ...
    rhdl::bits::W<T_W>: BitWidth,
{
    ...
    let shifted = q.extras.data_reg >> bits::<8>(1);
    ...
    match q.state {
        ...
        OneWireState::ResetLow => {
            if q.extras.tick == t.t_rst_low {
                d.state = OneWireState::ResetSample;
                next.tick = zero_t;
            }
        }
        OneWireState::ResetSample => {
            if q.extras.tick == t.t_rst_sample {
                next.presence_ok = !i.bus_in;
            }
            if q.extras.tick == t.t_rst_total {
                d.state = OneWireState::Stop;
                next.tick = zero_t;
            }
        }
        OneWireState::WriteBitLow => {
            let bit_is_one = (q.extras.data_reg & bits::<8>(1)) != bits::<8>(0);
            let pulse_width = if bit_is_one { t.t_w1 } else { t.t_w0 };
            if q.extras.tick == pulse_width {
                d.state = OneWireState::WriteBitWait;
                next.tick = zero_t;
            }
        }
        OneWireState::WriteBitWait => {
            if q.extras.tick == t.t_slot {
                if q.extras.bit_idx == bits::<4>(7) {
                    d.state = OneWireState::Stop;
                    next.tick = zero_t;
                } else {
                    next.bit_idx = q.extras.bit_idx + one_b4;
        ...
        OneWireState::ReadBitLow => {
            if q.extras.tick == t.t_read_low {
                d.state = OneWireState::ReadBitSample;
                next.tick = zero_t;
            }
        }
        OneWireState::ReadBitSample => {
            if q.extras.tick == t.t_read_sample {
                let bit_in = if i.bus_in {
                    bits::<8>(0x80)
                } else {
                    bits::<8>(0)
                };
                let cleared = q.extras.data_reg & bits::<8>(0x7F);
                next.data_reg = cleared | bit_in;
            }
            if q.extras.tick == t.t_slot {
                if q.extras.bit_idx == bits::<4>(7) {
                    d.state = OneWireState::Stop;
                    ...
                    next.bit_idx = q.extras.bit_idx + one_b4;
                    next.data_reg = q.extras.data_reg >> bits::<8>(1);
                    d.state = OneWireState::ReadBitLow;
                    next.tick = zero_t;
                }
            }
    ...
    if cr.reset.any() {
        d.state = OneWireState::Idle;
        next = OneWireMasterExtras::<T_W>::default();
    }
    ...
    o.bus_oe = bus_oe;
    o.bus_out = false;
    o.data_out = q.extras.data_reg;
    o.presence_ok = q.extras.presence_ok;
...
mod tests {
    ...
    fn test_one_wire_master_trace() -> miette::Result<()> {
        ...
        let expect = expect!["980112586f21be00155e21add5cef64a0a1d3bf657aeda49be1292fd582536b7"];
