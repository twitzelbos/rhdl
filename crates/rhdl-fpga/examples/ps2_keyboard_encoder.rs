...
fn main() -> Result<(), RHDLError> {
    ...
    let mut stream_in: Vec<In> = vec![In { scancode: bits(0), make: false, extended: false, send: false, clk_in: true }; 2];
    stream_in.push(In { scancode: bits(0x1C), make: true, extended: false, send: true, clk_in: true });
    for _ in 0..400 {
        stream_in.push(In { scancode: bits(0), make: false, extended: false, send: false, clk_in: true });
    }
