...
fn main() -> Result<(), RHDLError> {
    ...
    let idle = In { dx: signed::<9>(0), dy: signed::<9>(0), dz: signed::<4>(0), btn_left: false, btn_right: false, btn_middle: false, btn_4: false, btn_5: false, send: false, clk_in: true };
    let mut stream_in: Vec<In> = vec![idle; 2];
    let mut s = idle;
    s.dx = signed::<9>(7); s.dy = signed::<9>(-2); s.dz = signed::<4>(1); s.btn_middle = true; s.send = true;
    stream_in.push(s);
    for _ in 0..400 { stream_in.push(idle); }
