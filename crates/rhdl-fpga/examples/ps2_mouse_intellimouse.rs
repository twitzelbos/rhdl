...
fn main() -> Result<(), RHDLError> {
    ...
    for &b in &[0x08u8, 5, 10, 0x01, 0x09, 0xFE, 0xFF, 0x0F] {
        stream_in.push(In { byte_in: bits::<8>(b as u128), byte_valid: true });
        stream_in.push(In { byte_in: bits(0), byte_valid: false });
    }
    for _ in 0..6 { stream_in.push(In { byte_in: bits(0), byte_valid: false }); }
