...
fn higher_priority_task_wins_over_lower() {
    ...
    let final_state = trace.last().unwrap();
    assert_eq!(final_state.last_task.raw(), 14, "Disk Word should beat Emulator");
    assert_eq!(final_state.task_mpc[14].raw(), 114, "Disk Word's MPC should advance");
    assert_eq!(final_state.task_mpc[0].raw(), 0, "Emulator's MPC should NOT advance");
}
...
fn task_9_display_word_beats_task_0_emulator() {
    ...
    let final_state = trace.last().unwrap();
    assert_eq!(final_state.last_task.raw(), 9, "Display Word should beat Emulator");
    assert_eq!(final_state.task_mpc[9].raw(), 109);
...
fn task_14_disk_word_beats_task_4_disk_sector() {
    ...
    let uut = AltoTaskSystem::default();
    let trace = run(uut, input(WAKE_TASK_14_DISK_WORD | WAKE_TASK_4_DISK_SECT), 5);
    let final_state = trace.last().unwrap();
    assert_eq!(final_state.last_task.raw(), 14, "Disk Word should beat Disk Sector");
    assert_eq!(final_state.task_mpc[14].raw(), 114);
...
fn all_real_tasks_woken_picks_task_14() {
    ...
    let final_state = trace.last().unwrap();
    assert_eq!(final_state.last_task.raw(), 14, "Highest-priority task wins");
    assert_eq!(final_state.task_mpc[14].raw(), 114);
...
fn unused_task_slots_have_no_rules() {
    ...
    for unused_bit in [0x0004u16, 0x0008, 0x0020, 0x0040, 0x8000] {
        ...
        assert!(!trace.last().unwrap().any_running,
...
fn task_system_iverilog_round_trip() -> Result<(), RHDLError> {
    ...
    let real_wake: [u16; 10] = [
        WAKE_TASK_0_EMULATOR, WAKE_TASK_4_DISK_SECT, WAKE_TASK_7_ETHERNET,
        WAKE_TASK_8_MRT, WAKE_TASK_9_DISP_WORD, WAKE_TASK_10_CURSOR,
        WAKE_TASK_11_DISP_HOR, WAKE_TASK_12_DISP_VER, WAKE_TASK_13_PARITY,
        WAKE_TASK_14_DISK_WORD,
    ];
    let inputs: Vec<AltoIn> = (0..6).map(|cyc| input(real_wake[cyc % real_wake.len()])).collect();
