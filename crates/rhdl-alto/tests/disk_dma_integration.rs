...
fn disk_sector_mark_drives_disk_sector_task() {
    ...
    let arbiter_inputs: Vec<AltoIn> = disk_trace
        .iter()
        .map(|o| {
            let mut wake: u128 = 0;
            if o.sector_mark {
...
fn synthetic_word_strobe_drives_disk_word_task() {
    ...
    let arbiter_inputs: Vec<AltoIn> = (0..10)
        .map(|cycle| {
            if cycle < 5 {
...
fn disk_word_outranks_emulator_under_pressure() {
    ...
    let arbiter_inputs: Vec<AltoIn> = (0..10)
        .map(|cycle| {
            ...
            if cycle < 5 {
                wake |= 0x4000;
            } // disk word during transfer (bit 14)
            input_from_wakeups(wake)
        })
        .collect();
    ...
    for cyc in 1..=5 {
        assert_eq!(
            arbiter_trace[cyc].last_task.raw(),
            14,
