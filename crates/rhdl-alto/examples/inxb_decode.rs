...
fn main() {
    ...
    let interesting = [
        0x121, 0x129, 0x13a, 0x13b, 0x13c, 0x13d, 0x13e, 0x13f, 0x138, 0x139, 0x140, 0x141,
    ];
    for &mpc in &interesting {
        let word = microcode[mpc];
        let ui = Microinstruction::unpack(word);
        println!(
            "MPC=0x{:03x}: word=0x{:08x}  rsel={}  alu={:?}  bs={:?}  f1={:?}  f2={:?}  t_load={}  l_load={}  next=0x{:03x}",
            mpc,
            word,
            ui.rsel.raw(),
            ui.aluf,
    ...
    let probe_indices = [
        ...
        (
            41,
            "0x382 WriteKcomm: rsel=5,bs=1 → KCOM value (bit15=transfer_request)",
        ),
        (113, "0x383 LoadMar: rsel=14,bs=1"),
        (
            59,
            "0x13b LoadMar: rsel=7,bs=3 → MAR target (BS+F2=Constant overrides)",
        ),
    ];
    for &(idx, label) in &probe_indices {
        let v = crom[idx];
        println!(
            "  constant_rom[{:>3}] = 0x{:04x} ({}) — {}",
            idx,
            v,
            format!("bit15={}", (v >> 15) & 1),
            label
