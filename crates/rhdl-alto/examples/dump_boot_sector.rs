...
fn main() {
    ...
    println!("File size: 2,601,648 bytes (203 cyl × 2 head × 12 sec × 267 words × 2 bytes — matches Diablo 31 geometry)");
    println!();
    ...
    println!("  ratio non-zero:          {:.1}%", 100.0 * non_zero as f64 / 256.0);
    println!();
    if non_zero < 200 {
        ...
        println!("VERDICT: Looks like REAL period boot code (high non-zero density + reasonable opcode mix).");
    }
}
