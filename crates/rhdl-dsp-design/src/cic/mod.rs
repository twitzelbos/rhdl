#![warn(missing_docs)]
//! CIC design mathematics.
//!
//! Widths, gains, frequency response, pruning schedules and compensator
//! design for a cascaded integrator-comb decimator. All of it
//! design-time: nothing here builds hardware or knows what `Digital`
//! is.
//!
//! `rhdl-fpga` re-exports this tree from `dsp::cic` alongside the
//! widgets, so a caller sees one module.
...
pub(crate) const fn ceil_log2(v: usize) -> usize {
    let mut b = 0;
    while (1usize << b) < v {
        b += 1;
    }
